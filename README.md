...
### 安装依赖 [Node.js](https://nodejs.org/zh-tw)
```powershell
pnpm install
```
...
### 基本设定
- **`--content=all`** 預設是illust，all可能有漫畫，但圖片較全面(建議加上，尤其是2010前)
- **` --folder=test`** 創建一個資料夾放置本次下載的圖片
...
### ID 範圍下載模式
- **`--id-range=起始ID-結束ID`**: 指定作品 ID 範圍進行下載
- 例如: `--id-range=125000000-125001000` 會檢查 ID 125000000 到 125001000 的所有作品
- 不依賴日期，直接遍歷指定 ID 範圍內的作品
- 會創建格式為 `日期_idrange_起始ID-結束ID` 的資料夾
...
- **socket hang up**: 套接字掛斷（連接異常中斷）
---
- **重試次數**: 最多自動重試 3 次（**每張圖片獨立計算**）
- **重試延遲**: 遞增延遲（2秒 → 4秒 → 6秒）
- **超時時間**: 每個圖片下載 30 秒超時
...
- 图片 ID 对应的所有标签、排名、頁數
- 在执行的过程当中，black里面的txt,图片档案，原因不明的复制到上一层资料夹当中，在执行结束后就会复原，不要去对他做搬移或删除（這可能是 Windows 檔案系統或防毒軟體的暫存行為）
