...
```
3. 执行完成后进入"女の子_20250726"资料夹，检查里面是否有69个项目(67张图片)如果圖片少了，代表你的cookie是沒有登入的，沒辦法下載皮膚面積較大的圖片
...
```bash
# 只下载单图作品，跳过所有多图
node index.js 20240315 --skip-multi

# 遇到多图作品时只下载第一张
node index.js 20240315 --one

# 预设行为：下载多图作品的所有页面
node index.js 20240315

# 组合使用：下载指定标签，但跳过多图作品
node index.js --month=202403 --tag=初音ミク --skip-multi
```
```
- 会下载到picture资料夹中
...
### 功能开关
- **`--one`**: 仅下载多图作品的第一张图片
- **`--skip-multi`** 或 **`--no-multi`**: 完全跳过多图作品，只下载单图
- **`--all`**: 下载所有图片 (包括多图作品的所有页面，与预设行为相同)
- **`--nomanga`**: 检查是否长和宽相差很多(超过10倍)

#### 多图作品控制说明
- **预设行为**: 下载多图作品的所有页面
- **`--one`**: 遇到多图作品时只下载第一张图片
- **`--skip-multi`**: 遇到多图作品时完全跳过，不下载任何图片

**参数优先权**: 如果同时设定多个参数，优先权为：`--skip-multi` > `--one` > 预设行为
...
### 推荐的TAG
1. 较冷门图片: `--block=diaper,はいてな,腹肉,ぽっちゃり,bbw,SSBBW,肥満化,腹ワイパー,thicc,異形進化,髪行類,多脚ヤンヨ,ショタ,たくしあげ,へそ,腹筋,おちんちん,ルカメイ,男の子,創作男子,青年 `
