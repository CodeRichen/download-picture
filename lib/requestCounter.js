...
// 將下載請求加入統一佇列 (下載權重設為 5)
function enqueueDownload(requestFn) {
    requestQueue.push({ fn: requestFn, type: 'download', weight: 5 });
    processQueue();
}
...
function processQueue() {
    ...
    if (globalRequestCount + nextItem.weight > nextPauseAt) {
        console.log(`\n權重累積已達標 (${globalRequestCount}/${nextPauseAt})，暫停 ${PAUSE_DURATION/1000} 秒...\n`);
        isPaused = true;
    ...
    setTimeout(function() {
        // --- 根據類型增加不同權重 ---
        globalRequestCount += item.weight;
