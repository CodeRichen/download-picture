        ...
        counter.enqueueDownload(function(currentCount) {
            ...
            function attemptDownload() {
                ...
                .on('error', function(err) {
                    ...
                    // 檢查是否為可重試的錯誤
                    const retryableErrors = ['ESOCKETTIMEDOUT', 'ETIMEDOUT', 'ECONNRESET', 'ENOTFOUND', 'aborted'];
                    const isRetryable = retryableErrors.some(errType => err.message.includes(errType));
                    
                    if (isRetryable && retryCount < maxRetries) {
                        retryCount++;
                        console.log(`[重試${retryCount}/${maxRetries}] ${name} - ${err.message}`);
                        fileStream.destroy();
                        fs.unlink(filePath, function() {
                            setTimeout(attemptDownload, 2000 * retryCount); // 遞增延遲：2s, 4s, 6s
                        });
                    } else {
                        console.log(`[錯誤] ${name} 下載失敗: ${err.message}${retryCount > 0 ? ` (已重試${retryCount}次)` : ''}`);
                        hasError = true;
                ...
                fileStream.on('finish', function() {
                    if (!downloadAborted) {
                        ...
                        if (expectedSize > 0 && downloadedSize < expectedSize) {
                            ...
                            console.log(`[不完整] ${name} 只下載了 ${percentage}% (${downloadedSize}/${expectedSize} bytes)`);
                            
                            // 視為下載失敗，嘗試重試
                            if (retryCount < maxRetries) {
                                retryCount++;
                                console.log(`[重試${retryCount}/${maxRetries}] ${name} - 文件不完整`);
                                fs.unlink(filePath, function() {
                                    setTimeout(attemptDownload, 2000 * retryCount);
                                });
                            } else {
                                console.log(`[錯誤] ${name} 下載失敗: 文件不完整 (已重試${retryCount}次)`);
                                hasError = true;
                                fs.unlink(filePath, function() {
                                    markAsCompleted(false);
                                });
