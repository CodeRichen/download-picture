    ...
    urls.forEach((url, index) => {
        counter.enqueueDownload(function(currentCount) {
            ...
            fileStream.on('finish', function() {
                downloadedCount++;
                console.log(`[${date}] ${name}_${index} 下載完成 `);

                // 4. 當所有圖片都下載完成（或單圖下載完成）才寫入 tags.txt
                if (downloadedCount === urls.length) {
                    writeTagRecord(tagsTxtPath, name, date, tags);
                }
            });
        });
    });
...
// 輔助函式：寫入標籤資訊
function writeTagRecord(filePath, name, date, tags) {
    const tagStr = (tags && tags.length > 0) ? tags.join(", ") : "無標籤";
    // 格式：日期 | ID: 名稱 | Tags: 標籤
    const logEntry = `[${date}] ID: ${name} | Tags: ${tagStr}\n`;
