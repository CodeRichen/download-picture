...
function getImgUrl(content, cookie, date, options) {
    ...
    function processNext() {
        ...
        fetchIllust(id, function(err, urls, tags, skippedByOne, illustInfo) {
            ...
            if (!err && urls) {
                if (!skippedByOne) {
                    // 從 options.pageMap 取得該 ID 的 metadata（包含 isBlocked）
                    var pageInfo = options.pageMap && options.pageMap[id] ? options.pageMap[id] : {};
                    var isBlocked = pageInfo.isBlocked || false;
                    
                    if (isBlocked) {
                        console.log(`[getImgUrl] ID: ${id} isBlocked = true`);
                    }
                    
                    // 傳遞下載成功回調給 save
                    var saveOptions = Object.assign({}, options);
                    saveOptions.onDownloadSuccess = function(illustId, status, downloadTags, saveMetadata) {
                    ...
                    saveOptions.isBlocked = isBlocked;
