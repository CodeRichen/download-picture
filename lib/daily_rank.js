...
function daily_rank(cookie, date, options, sharedCache) {
    ...
    if (options && options.block) {
        ...
        // console.log("設定屏蔽標籤（智能匹配）:", blockTags.join(", "));
    }
    
    // 處理 noword block 標籤列表（強制部分匹配）
    var nowordBlockTags = [];
    if (options && options.nowordBlock) {
        nowordBlockTags = options.nowordBlock.split(",").map(tag => tag.trim().toLowerCase());
        // console.log("設定屏蔽標籤（強制部分匹配）:", nowordBlockTags.join(", "));
    }
    ...
    var useSharedCache = !!sharedCache;
...
function processPageData(data, page, done) {
    ...
    data.contents.forEach(function(item) {
        var illustId = String(item.illust_id); // 這裡的 ID 對應name

        // 檢查重複：判斷該 ID 是否已存在於 tags.txt
        if (existingTagsContent && existingTagsContent.includes(`ID: ${illustId} |`)) {

            // console.log(`[跳過] ${illustId} 已存在於 tags.txt`);
            return; 
        }
        //檢查是否在失敗清單中
        ...
        if (options.tag) {
            ...
            if (!isBlocked && options.blockGroups && options.blockGroups.length > 0) {
                for (var g = 0; g < options.blockGroups.length; g++) {
                    ...
                    // 檢查這個群組中的「每一個」標籤是否都出現在作品標籤中
                    var allMatch = currentGroup.every(function(memberTag) {
                        // 使用 indexOf 檢查是否包含該標籤 (部分匹配)
                        return tagsStr.indexOf(memberTag) !== -1;
                    });

                    if (allMatch) {
                        isBlocked = true; // 全部都中了，這張圖要擋掉
                        // console.log(`[群組過濾] ID: ${illustId} 包含完整群組 ${currentGroup}`);
                        break; 
                    }
                }
            }
            if (!isBlocked && options.conditionalBlocks && options.conditionalBlocks.length > 0) {
                    for (var c = 0; c < options.conditionalBlocks.length; c++) {
                        ...
                        // 檢查 A 是否存在
                        var hasA = tagsStr.indexOf(rule.ifExists) !== -1;
                        // 檢查 B 是否存在
                        var hasB = tagsStr.indexOf(rule.mustHave) !== -1;

                        // 邏輯核心：當有 A 但沒有 B 時，判定為 Blocked
                        if (hasA && !hasB) {
                            isBlocked = true;
                            // console.log(`[條件過濾] ID: ${illustId} 有 ${rule.ifExists} 但沒 ${rule.mustHave}，屏蔽`);
                            break;
                        }
                    }
                }
    ...
    function fetchNext(page) {
        if (page > pages) {
            // 保存缓存（如果有更新且不是共享緩存模式）
            if (cacheUpdated && !useSharedCache) {
